      ...
      <div className="space-y-10">
        {appointments?.length > 0 ? (
          Object.entries(groupedAppointments).map(([date, apps]) => (
            <div key={date} >
              ...
              <div className=" top-20 z-10 mb-4">
                <h3 className="inline-block bg-green-500 text-white px-4 py-1 rounded-lg text-sm font-bold shadow-md">
                  {date}
                </h3>
              </div>
                    ...
                    <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                      <div>
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className="bg-indigo-50 text-green-600 text-s font-bold px-2 py-1 rounded uppercase tracking-wider">
                            {app.service.name}
                          </span>
                        </div>
          ...
          <div className="bg-white rounded-3xl p-20 text-center border-2 border-dashed border-slate-200">
            <p className="text-slate-400 text-lg font-medium italic">Nema rezerviranih termina danas</p>
          </div>
        )}
      </div>
    </div>
